...
The background Dock badge uses only `brew outdated --json=v2`. It does not run `brew update` or any package mutation. Tap names must match a strict ASCII `owner/repository` format before the Rust runner receives them.
