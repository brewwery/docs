...
- Filter formulae by category: all, Homebrew Leaves, installed on request, or dependencies
- Search through installed packages locally
- Sort by name, version, or status
- Detail drawer with dependencies and installed dependents
- Virtualized rows with Arrow, Home, End, Enter, and Space keyboard controls

### Taps

Manage additional Homebrew repositories safely:

- List installed official and third-party taps
- Add taps using a validated `owner/repository` name
- Remove taps after explicit confirmation
- Keep tap and untap results in local History
...
Save your most-used packages for quick access:

- Mark any formula or cask as a favorite from the package card or detail drawer
- Favorites appear in a dedicated Favorites view in the sidebar
- Favorites persist across sessions via local settings
- Favorite badge shown inline on package cards and search results
...
- Show the read-only outdated count in the macOS Dock badge during background checks
...
Background checks never run `brew update` or mutate installed packages.
